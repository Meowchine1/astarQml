...
QVariantList AppCore::startAlgorithmWithAutogeneratedGraph(int start_x, int start_y, int finish_x, int finish_y){
    try{
        Node* fromNode =  randomGraph->getNode(start_y, start_x);
        Node* toNode =  randomGraph->getNode(finish_y, finish_x);
        QVariantList minway = astar->run(fromNode, toNode, randomGraph);
        return  minway;
    }
    catch(NodeException ex){
        qDebug()<<ex.getMessage();
...
QVariantList AppCore::getVisited(){
    QVariantList result;
    std::vector<Node*> visitedNodes = astar->getVisited();
    for(auto node: visitedNodes){

        result.append(node->getX()); result.append(node->getY());
